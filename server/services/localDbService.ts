...
export class LocalDatabase {
  ...
  constructor() {
    this.filePath = path.join(SERVER_ROOT, 'data_store.json');
    this.data = {
      users: [],
      tickets: [],
      comments: [],
  ...
  private seedSenaiItemsIfNeeded() {
    try {
      if (!this.data.availableItems || this.data.availableItems.length === 0) {
        const baseCsvPath = path.join(SERVER_ROOT, 'Consulta de Itens - Atas e Contratos - Agosto 3(BASE DE DADOS).csv');
